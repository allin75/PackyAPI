...
排行榜在自然整点过期，由整点后的第一个访问请求触发刷新，不运行后台定时任务。单个账号访问失败时应标记为“沿用上轮”，并继续刷新其他账号。

服务每天北京时间 23:00 为全部账号各执行一次顺序查询，将累计用量和每日差值写入 `daily_usage_snapshots`。每个账号每天只自动查询一次，失败不重试且不影响其他账号。`/my` 的完整历史仍只对当前 Cookie 会话可见；公开日榜仅返回主动参榜账号在所选今日/昨日的用量，不返回完整历史或余额。缺失用量与名次为 null。查询失败日志仅含错误类型、HTTP 状态及时间，不记录 Key、账号或响应正文。

验证 `/api/leaderboard?period=today`、`?period=yesterday`、`?period=all` 三个榜单；今日与累计按整点刷新，昨日只读每日记录。今日 23:00 后展示当日结算记录，午夜切到新日期，不沿用前日排名或失败兜底。切换参榜状态应立即生效，包括刷新进行中的退出。

## 5. 备份和升级

- 备份前停止容器，再备份整个 `data` 目录和单独保存的主密钥。
- 本版本会将 SQLite `user_version` 自动迁移到 `5`；升级后应确认 `daily_usage_snapshots` 与 `today_leaderboard_state` 表存在。后者只存本日最新一轮排行，以保留重启后的名次变化。迁移保留账号、会话和每日记录；回退旧版服务前需还原升级前备份，旧版不支持版本 5。
