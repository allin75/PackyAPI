...
服务每天北京时间 23:00 为全部账号各执行一次顺序查询，将累计用量和每日差值写入 `daily_usage_snapshots`。每个账号每天只自动查询一次，失败不重试且不影响其他账号。`/my` 的“昨日用量”只对当前 Cookie 会话可见，排行榜接口不应包含每日字段。

## 5. 备份和升级

- 备份前停止容器，再备份整个 `data` 目录和单独保存的主密钥。
- 本版本会将 SQLite `user_version` 自动迁移到 `4`；升级后应确认 `daily_usage_snapshots` 表存在。
