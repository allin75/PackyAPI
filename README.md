...
- `/leaderboard` 使用赛事转播式领奖台和紧凑榜单，显示相较上一轮整点周期的名次变化。
- 排行榜 API 只公开名称、已用金额、名次和变化状态，不公开账号 ID、Key、IP 或剩余额度。
- 排行榜在整点后由第一个访问请求触发，逐个刷新账号以避免上游突发限流；单个账号失败时沿用其上轮用量，不影响其他账号刷新。
- 同一个浏览器会话可以绑定多个 Key；不同浏览器会话互相隔离，即使共用同一个出口 IP。
- 服务每天北京时间 23:00 为每个账号采集一次累计用量，保存每日差值；`/my` 显示当前会话账号的昨日用量。
- 旧 IP 绑定记录会保留用于迁移和审计，但不再作为私有页面授权依据。
- Key 使用 AES-256-GCM 加密保存在 SQLite 中，不会下发到浏览器。
- 私有页与排行榜共用每账号至少 5 分钟的请求缓存；普通页面关闭后不刷新，仅保留每天一次的用量边界采集。
...
测试覆盖多 Key/多会话隔离、排行榜字段过滤、数据库迁移、整点快照、每日 23:00 快照、昨日用量、名次升降、并发请求合并、逐账号失败回退、会话解绑、可信代理解析、HTTPS 限制和登记限流。
